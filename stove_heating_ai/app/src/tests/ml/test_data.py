...
def sample_data():
    ...
    dates = pd.date_range(
        start=datetime(2024, 1, 1), end=datetime(2024, 1, 1, 1, 59), freq="min"
    )
    ...
    episode1_mask = (data.index >= "2024-01-01 00:10:00") & (
        data.index < "2024-01-01 00:30:00"
    )
    ...
    episode2_mask = (data.index >= "2024-01-01 01:00:00") & (
        data.index < "2024-01-01 01:15:00"
    )
    data.loc[episode2_mask, "stove_status"] = 1
    temps2 = np.linspace(
        18.0, 20.0, 15
    )  # 15 minutes from 18°C to 20°C (doesn't reach setpoint)
...
def test_extract_heating_episodes_basic(sample_data):
    ...
    assert all(
        y_values.iloc[i] > y_values.iloc[i + 1] for i in range(len(y_values) - 1)
    )
